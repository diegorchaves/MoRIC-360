# === O que verificar na implementação do SWHDC ===
# Para ser um drop-in replacement de nn.Conv2d, ela precisa ter:

# self.weight e self.bias acessíveis (por causa das inicializações with torch.no_grad() logo após a instanciação)
# forward(x) recebendo um tensor (B, C_in, H, W) e retornando (B, C_out, H', W')
# ===
...
class LocalGlobalBlock(LocallyConnectedBlock):
    def __init__(
        ...
        self.agg_func.append(
            nn.Sequential(SynthesisLayer(global_hid_channels + 6, 3, 1, nn.GELU()))
        )
        self.agg_func.append(
            nn.Sequential(SynthesisLayer(global_hid_channels + 9, 3, 1, nn.GELU()))
        )
        self.agg_func.append(
            nn.Sequential(SynthesisLayer(global_hid_channels + 12, 3, 1, nn.GELU()))
        )
        self.agg_func = nn.Sequential(*self.agg_func)

        self.full_net = []
        self.full_net.append(
    ...
    def forward(self, coordinate, combined_latent):
        ...
        for local_layer, global_layer, agg_layer in zip(
            ...
            if id < 3:
                ...
                # === Ponto de atenção, verificar o shape do mask antes de depois desse squeeze ===

...
def train_single_image(args, image_index, trainer):
    ...
    # Evaluation
    psnr_eval, bits_rate, bits_rate_num, _ = trainer.evaluate(target_mask_flat, dataloader)
    
    print(f"Evaluation - Image {image_index}: PSNR={psnr_eval:.6f}, Rate={bits_rate:.6f}")
    ...
    (eval_psnr, eval_y_rate, eval_y_rate_num,
     eval_network_rate, eval_network_rate_num,
     eval_arm_rate, eval_arm_rate_num,
     eval_conv_rate, eval_conv_rate_num) = eval_model(
        target_mask_flat, args, model, None, dataloader, image_index
    )
    ...
    print(f"  Y rate: {results['y_rate']:.6f}, Network rate: {results['network_rate']:.6f}, Border: {results['border_rate']:.6f}")
    print(f"  ARM rate: {results['arm_rate']:.6f}, Conv rate: {results['conv_rate']:.6f}")
...
def run_training_loop(args):
    ...
    for lambda_idx, lambda_rate in enumerate(args.lambda_rate_list):
        print(f"\n\n{'#'*70}")
        print(f"# Lambda rate {lambda_idx + 1}/{len(args.lambda_rate_list)}: {lambda_rate}")
        print(f"{'#'*70}\n")
        ...
        if lambda_results['images']:
            ...
            lambda_results['avg_total_rate'] = np.mean(lambda_results['total_rate_list'])
